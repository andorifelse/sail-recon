...
def setup_args():
    ...
    parser.add_argument(
        "--recon", type=int, default=5, help="Number of reconstruction views"
    )
    parser.add_argument(
        "--reloc", type=int, default=5, help="Number of relocation views"
    )
    parser.add_argument(
        "--fixed_rank", type=int, default=300, help="Fixed rank for VGGT model"
    )
...
def process_sequence(
    ...
    views = [
        {"img": images[i : i + 1]}  # shape (1, 3, 518, 518)
    ...
    print(
        f"Reconstruction views: {recon}, Relocation views: {reloc}, Fixed rank: {fixed_rank}"
    )
    ...
    if use_ba:
        try:
            pred_extrinsic = run_vggt_with_ba(
                model, images, image_names=image_names, dtype=dtype
            )
...
def main():
    ...
    for category in SEEN_CATEGORIES:
        ...
        mean_AUC_30_by_now = np.mean(
            [
                per_category_results[category]["Auc_30"]
                for category in per_category_results
            ]
        )
        mean_AUC_15_by_now = np.mean(
            [
                per_category_results[category]["Auc_15"]
                for category in per_category_results
            ]
        )
        mean_AUC_5_by_now = np.mean(
            [
                per_category_results[category]["Auc_5"]
                for category in per_category_results
            ]
        )
        mean_AUC_3_by_now = np.mean(
            [
                per_category_results[category]["Auc_3"]
                for category in per_category_results
            ]
        )
        Racc_15_by_now = np.mean(
            [
                per_category_results[category]["Racc_15"]
                for category in per_category_results
            ]
        )
        Tacc_15_by_now = np.mean(
            [
                per_category_results[category]["Tacc_15"]
                for category in per_category_results
            ]
        )
        t_process_by_now = np.mean(
            [
                per_category_results[category]["time"]
                for category in per_category_results
            ]
        )
        print(
        ...
        print(
            f"{BOLD}{BLUE}Average time of processing each sequence:{RESET} {RED}{t_process_by_now:.2f} ms{RESET}"
        )
        print("=" * 80)
    ...
    if per_category_results:
        ...
        mean_Racc_15 = np.mean(
            [
                per_category_results[category]["Racc_15"]
                for category in per_category_results
            ]
        )
        mean_Tacc_15 = np.mean(
            [
                per_category_results[category]["Tacc_15"]
                for category in per_category_results
            ]
